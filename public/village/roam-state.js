...
export function createGame(mode='easy'){
 ...
 return {mode,settings,phase:'ready',player:{x:650,y:770,facing:1,walk:0,vx:0,dx:0,dy:0},cats:[[270,310],[1080,340]].slice(0,settings.cats).map(([x,y],id)=>({id,x,y,facing:1,route:[],repath:0})),boost:0,protection:0,timeBonusEarned:0,lastTimeBonus:0,hazards:[],nextHazard:8,streak:0,streakUntil:0,powers:powerSpots.map(([x,y],i)=>({x,y,type:[1,5,9].includes(i)?'shield':'speed',readyAt:i===8?5:i===9?9:0})).filter(p=>mode!=='hard'||p.type==='speed'),remaining:60,score:0,found:0,elapsed:0,next:3,reason:null,friends:spots.slice(0,3).map(([x,y],id)=>({id,x,y})),target:null,route:[]};
}
...
export function step(s,axes,dt){if(!['playing','explore'].includes(s.phase)||!Number.isFinite(dt)||dt<=0)return [];const playing=s.phase==='playing',events=[];if(s.mode==='hard')s.protection=0;if(playing){s.remaining=Math.max(0,s.remaining-dt);s.elapsed+=dt;s.boost=Math.max(0,s.boost-dt);if(!s.remaining){s.phase='ended';s.reason='time';s.player.vx=0;return ['end'];}}
