...
function render(lang, route) {
  ...
  if (route === "" || route === "village") {
    ...
    body = `<main id="main" class="world ${route === "" ? "entrance" : "village"}"><div class="village-canvas"><img class="map-art" src="/assets/village.jpg" alt="${t.artAlt}" width="1536" height="1024" fetchpriority="high">${route === "" ? `<div class="invitation"><span class="whisper">${t.welcome}</span><h1>${t.found.replace("\n", "<br>")}</h1><p>${t.intro}</p><a class="knock-button" href="${url(lang, "village")}" data-knock data-opening="${t.opening}">${t.enter} <span aria-hidden="true">↪</span></a></div>` : `<div class="map-title"><h1>${t.title}</h1><p>${t.villageNote}</p></div><nav class="houses" aria-label="${t.mapTitle}">${destinations.map((r, i) => `<a class="house house-${i}" href="${url(lang, r)}"><span class="house-label"><span class="house-number">${i + 1}</span>${t.labels[routes.indexOf(r)]}<span class="house-arrow" aria-hidden="true">↗</span></span><span class="house-caption">${t.signs[i]}</span></a>`).join("")}</nav><a class="cinema-sign" href="${url(lang, "media")}">${icon("play")}${t.videoShortcut} ↗</a>`}</div></main>`;
  } else {
    let inside = "";
    if (route === "about")
      inside =
    ...
    if (route === "credits")
      inside =
        heading(t.labels[current]) +
        `<div class="credits-note"><h2>${lang === "fr" ? "Les Schtroumpfs" : "The Smurfs"}</h2><p>${t.creditsText}</p><p>${t.artCredit}</p><p>${t.sourceCredit}</p><a class="ink-link" href="https://robo-crc.ca/${lang}">${t.crcLink}</a></div>`;
  ...
  return `<!DOCTYPE html><html lang="${lang}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="theme-color" content="#f3efd9"><title>${escape(t.labels[current])} — ${escape(t.title)}</title><meta name="description" content="${escape(t.entryNote)}"><link rel="alternate" hreflang="${other}" href="${url(other, route)}"><link rel="icon" href="/assets/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="/styles.css"><script src="/app.js" defer></script></head><body class="${route === "" || route === "village" ? "map-page" : "room-page"}"><a class="skip" href="#main">${t.skip}</a><header class="site-header"><a class="school-brand" href="${url(lang, "village")}"><span class="school-stamp">${t.stamp}</span><span>${t.school}<small>${t.subject}</small></span></a><div class="header-actions"><button class="map-button" type="button" data-open-map aria-haspopup="dialog">${t.menu} <span aria-hidden="true">☷</span></button><a class="locale-button" href="${url(other, route)}" lang="${other}" hreflang="${other}" aria-label="${t.localeLabel}">${t.locale}</a></div></header>${body}<footer class="site-footer"><span>${t.entryNote}</span><a href="${url(lang, "credits")}">${t.creditShortcut}</a></footer><dialog id="map-dialog" aria-labelledby="map-dialog-title"><button class="close-map" type="button" data-close-map aria-label="${t.close}">×</button><h2 id="map-dialog-title">${t.mapTitle}</h2><nav aria-label="${t.menu}">${routes
    .slice(1)
    .map(
      (r) =>
        `<a href="${url(lang, r)}"${r === route ? ' aria-current="page"' : ""}>${t.labels[routes.indexOf(r)]}<span aria-hidden="true">↗</span></a>`,
    )
    .join("")}</nav></dialog></body></html>`.replace(
    /&(?!amp;|lt;|gt;|quot;|#39;)/g,
    "&amp;",
  );
}
