...
function render(lang, route) {
  ...
  if (route === "" || route === "village") {
    ...
    const mapArt = `<canvas id="village-ground" aria-hidden="true"></canvas>`;
    body = `<main id="main" class="world ${route === "" ? "entrance" : "village"}"><div class="village-canvas">${mapArt}<nav class="houses" aria-label="${t.mapTitle}">${destinations.map((r,i)=>`<a class="house house-${i}" href="${url(lang,r === "game" ? "play" : r)}"${r === "game" ? " data-full-page" : ""}><canvas class="house-art" data-house="${i}" aria-hidden="true"></canvas><span class="house-label">${t.labels[routes.indexOf(r)]}</span></a>`).join('')}</nav><div class="map-title"><img class="centered-school-logo" src="/assets/sainte-anne-crest.svg" alt="Sainte-Anne" width="90" height="100"><h1>${t.title}</h1>${route === '' ? `<a class="knock-button" href="${url(lang,'village')}" data-knock data-opening="${t.opening}">${t.enter}</a>` : ''}</div><a class="cinema-sign" href="${url(lang,'media')}">${icon('play')}${t.videoShortcut}</a></div></main>`;
