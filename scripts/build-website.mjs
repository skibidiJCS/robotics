...
function render(lang, route) {
  ...
  if (route === "" || route === "village") {
    ...
    const mapArt = `<picture class="map-picture"><source media="(min-aspect-ratio: 4/3)" srcset="/assets/village-wide.jpg" width="2172" height="724"><img class="map-art" src="/assets/village.jpg" alt="${t.artAlt}" width="1536" height="1024" fetchpriority="high"></picture>`;
    const map = route === "" ? `<div class="invitation"><img class="centered-school-logo" src="/assets/sainte-anne-logo.svg" alt="Sainte-Anne" width="176" height="176"><span class="whisper">${t.welcome}</span><h1>${t.found.replace("\n", "<br>")}</h1>${t.intro ? `<p>${t.intro}</p>` : ""}<a class="knock-button" href="${url(lang, "village")}" data-knock data-opening="${t.opening}">${t.enter}</a></div>` : `<div class="map-title"><img class="centered-school-logo" src="/assets/sainte-anne-logo.svg" alt="Sainte-Anne" width="176" height="176"><h1>${t.title}</h1></div><nav class="houses" aria-label="${t.mapTitle}">${destinations.map((r, i) => `<a class="house house-${i}" href="${url(lang, r)}"><span class="house-label">${t.labels[routes.indexOf(r)]}</span></a>`).join("")}</nav><a class="cinema-sign" href="${url(lang, "media")}">${icon("play")}${t.videoShortcut}</a>`;
    body = `<main id="main" class="world ${route === "" ? "entrance" : "village"}"><div class="village-canvas">${mapArt}${map}</div></main>`;
    ...
    body = `<main id="main" class="room-world"><section class="room room-${route}" aria-label="${escape(t.labels[current])}"><a class="room-back" href="${url(lang, "village")}">← ${t.back}</a>${inside}${route === "game" ? `<a class="knock-button game-entry" href="${url(lang,"play")}" data-full-page>${lang === "en" ? "Play Escape Azrael" : "Jouer à Échappez à Azraël"} →</a>` : ""}</section></main>`;
  }
